...
class StudentController extends Controller
{
    ...
    public function getIrregularEnrollmentSubjects(Request $request)
    {
        try {
            $user = Auth::guard('student')->user();
            $student = $user->user_information->student;
            
            // Get available subjects for irregular students
            $subjects = $this->getAvailableSubjectsForIrregular($student, $student->year_level, '2nd Sem', []);
            
            // Format response
            $formattedSubjects = $subjects->map(function($subject) use ($student) {
                $hasPrerequisites = $subject->prerequisites && $subject->prerequisites->count() > 0;
                $prerequisitesMet = $hasPrerequisites ? 
                    $subject->prerequisites->every(function($prereq) use ($student) {
                        // Check if prerequisite is completed
                        return DB::table('enrolled_sub')
                            ->where('student_id', $student->id)
            ...
            return response()->json([
                ...
                'total_units' => 23, // Set your max units
                'year_level' => $student->year_level,
                'semester' => '2nd Sem'
            ]);
            
        } catch (\Exception $e) {
            Log::error('Get irregular enrollment subjects error: ' . $e->getMessage());
            return response()->json([
                'success' => false,
                'message' => 'Failed to load enrollment data'
            ]);
    ...
    public function enrollIrregularSubjects(Request $request)
    {
        try {
            $user = Auth::guard('student')->user();
            $student = $user->user_information->student;
            ...
            // Use your existing enrollment logic here
            return $this->enrollSubjects($request);
            
        } catch (\Exception $e) {
            Log::error('Irregular enrollment error: ' . $e->getMessage());
            return response()->json([
                'success' => false,
                'message' => 'Enrollment failed. Please try again.'
            ]);
