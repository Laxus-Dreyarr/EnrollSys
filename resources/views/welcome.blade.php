...
<script>
(function() {
    // Function to send theme info to React Native WebView
    function sendThemeToReactNative() {
        // Only proceed if inside a React Native WebView
        if (!window.ReactNativeWebView) return;

        const isDark = document.body.classList.contains('dark-theme');
        const theme = isDark ? 'dark' : 'light';
        
        // Get computed background color (optional – useful for setting the exact status bar color)
        const bgColor = getComputedStyle(document.body).backgroundColor;

        window.ReactNativeWebView.postMessage(JSON.stringify({
            type: 'themeChange',
            theme: theme,
            backgroundColor: bgColor   // e.g. "rgb(16, 17, 38)"
        }));
    }

    // 1. Send initial theme once the page is fully loaded
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', sendThemeToReactNative);
    } else {
        sendThemeToReactNative();
    }

    // 2. Hook into your existing theme toggle button
    const themeToggleBtn = document.getElementById('themeToggle');
    if (themeToggleBtn) {
        // We wrap the original click handler – keep your existing one intact
        const originalClick = themeToggleBtn.onclick;
        themeToggleBtn.addEventListener('click', function(e) {
            // Let the original handler run first (it toggles the class)
            if (originalClick) originalClick.call(this, e);
            
            // Wait a tiny bit for the class to be applied, then notify RN
            setTimeout(sendThemeToReactNative, 50);
        });
    }

    // 3. Observe class changes on <body> (in case theme changes programmatically)
